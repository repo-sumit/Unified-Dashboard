 ...
 * chip) · one inline value+descriptor row ("4.7K students absent…") with the N+1
 * peer comparison (+ allowed delta) right-aligned. Shares the same `KpiInlineRow`
 * grammar as multi-metric cards so single/multi cards align across the grid. No
 * source, no metric-type labels, no "Parent avg". Date context lives in the
 * header chip only, so it never repeats in the body.
 */
export function KpiCard({
  ...
  const descriptor = formatKpiCardTitlePhrase(kpi.name, kpi.name_gu, kpi.unit, lang);
  ...
  return (
    <KpiCardShell onClick={onClick} compare={<KpiCompareSection kpi={kpi} />}>
      ...
      {/* inline value + descriptor · N+1 comparison (+ allowed delta) right-aligned */}
      <div className="mt-2">
        <KpiInlineRow
          descriptor
