...
const RAW: Array<Partial<CatItem> & Pick<CatItem, "id" | "domain_id" | "name" | "name_gu" | "unit" | "direction" | "data_source">> = [
  ...
  { id: "att_chronic", domain_id: "attendance", name: "Chronic absentee students (7 consecutive days)", name_gu: "સતત ગેરહાજર વિદ્યાર્થીઓ (7 સળંગ દિવસ)", unit: "count", direction: "lower", data_source: ATT, frequency: "Daily", displayStrategy: "count_with_rate", hero: true, formula: "Students Absent 7 Consecutive Days / Total Enrolled × 100", description: "Absent 7 consecutive days — shown as a count and a rate." },
  ...
  { id: "asm_remediation", domain_id: "assessment", name: "SAT report downloaded in classroom", name_gu: "વર્ગખંડમાં SAT રિપોર્ટ ડાઉનલોડ", unit: "%", direction: "higher", data_source: "Gyan Prabhav MVs", frequency: "Monthly", displayStrategy: "compliance", hero: true, formula: "Classrooms where the SAT report was downloaded / Total classrooms × 100", description: "Gyan Prabhav generates a SAT report card per classroom; this is the share of classrooms that downloaded it (data-driven remediation)." },
  ...
  { id: "ret_reenroll", domain_id: "administration", sub_domain: "adm_retention", name: "Re-enrolment of OoSC (Out of School) against target %", name_gu: "OoSC (શાળા બહારના) ની લક્ષ્ય સામે પુનઃનોંધણી %", unit: "%", direction: "higher", data_source: "Back-to-School Bot · CTS/EWS", availableInDataLake: false, frequency: "Half yearly", displayStrategy: "compliance", hero: true, lowestLevel: "school", dataLagNote: "Half-yearly; confirmed at the start of the next year from CTS, so expect a data lag.", formula: "Students Enrolled via Back-to-School / Total Flagged × 100" },
